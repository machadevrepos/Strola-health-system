...
// Run with: node scripts/hardknock-audit.mjs (dev server must be on :3200)
...
const run = async () => {
  ...
  // ---- Fleet (super-admin exclusive) ----
  await withPage(browser, "/fleet", async (page) => {
    await page.goto(BASE + "/fleet", { waitUntil: "networkidle" });
    await page.waitForTimeout(300);

    await safeClick(page, "/fleet", "button:has-text('Provision device')");
    await page.waitForTimeout(200);
    await pressEscape(page);

    await safeClick(page, "/fleet", "button[aria-label='Force unpair']");
    await page.waitForTimeout(200);
    await safeClick(page, "/fleet", "button:has-text('Cancel')");
  });

  // ---- Staff & Roles (super-admin exclusive) ----
  await withPage(browser, "/staff", async (page) => {
    await page.goto(BASE + "/staff", { waitUntil: "networkidle" });
    await page.waitForTimeout(300);

    // promote flow: open, search, select a candidate, pick a role, cancel (never confirm)
    await safeClick(page, "/staff", "button:has-text('Promote a user')");
    await page.waitForTimeout(200);
    await page.locator("input[placeholder*='Search']").first().fill("Tom").catch((e) => record("/staff", "interaction-error", `search fill: ${e.message}`));
    await page.waitForTimeout(300);
    await safeClick(page, "/staff", "button:has-text('Tom Brennan')");
    await page.waitForTimeout(150);
    await safeClick(page, "/staff", "button:has-text('Cancel')");

    // existing staff row: role select -> confirm dialog -> cancel. Use the
    // last row (most recently promoted, role "admin") and target "Super
    // admin" — picking a role the row doesn't already have, since selecting
    // the current value is a no-op (requestRoleChange guards against it)
    // and never opens a confirm dialog to cancel.
    const roleTriggers = page.locator("table [data-slot='select-trigger']");
    if (await roleTriggers.count() > 0) {
      await roleTriggers.last().click();
      await page.waitForTimeout(200);
      await safeClick(page, "/staff", "[role='option']:has-text('Super admin')");
      await page.waitForTimeout(200);
      await safeClick(page, "/staff", "button:has-text('Cancel')");
    } else {
      record("/staff", "missing-element", "no staff role selects found");
    }

    // remove-from-staff: open confirm, cancel
    await safeClick(page, "/staff", "button:has-text('Remove')");
    await page.waitForTimeout(200);
    await safeClick(page, "/staff", "button:has-text('Cancel')");
  });
