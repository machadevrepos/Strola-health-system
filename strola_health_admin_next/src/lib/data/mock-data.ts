...
// day for "Premium started" in the profile view.
...
const FEATURED_DAILY_HISTORY_USERS = new Set(["usr_001", "usr_003", "usr_006", "usr_012"]);

// Every other real (non-deleted) user gets a lighter, programmatically
// varied history too — otherwise "Lifetime stats" on their profile would be
// empty for all but four demo accounts.
const otherDailyHistoryUsers = mockUsers.filter((u) => u.role === "user" && !u.deleted && !FEATURED_DAILY_HISTORY_USERS.has(u.id));
...
export const mockDailySummaries: DailyActivitySummary[] = [
  ...
  ...otherDailyHistoryUsers.flatMap((u, i) =>
    buildDailyHistory(u.id, 5500 + ((i * 733) % 5500), 900 + ((i * 211) % 1100), 30)
  ),
];
...
export const mockReports: Report[] = [
  ...
  { id: "rep_007", reporter_id: "usr_005", target_type: "post", target_id: "post_011", category: "advertising", reason: "Same 'free premium' spam I've seen on three other posts this week.", status: "open", action_taken: null, resolved_by: null, resolved_at: null, resolution_note: null, created_at: daysAgo(0, 12, 0) },
  { id: "rep_008", reporter_id: "usr_010", target_type: "post", target_id: "post_011", category: "spam", reason: "Clicked the link out of curiosity, it's definitely a phishing page.", status: "open", action_taken: null, resolved_by: null, resolved_at: null, resolution_note: null, created_at: daysAgo(0, 14, 0) },
];
...
export const mockChallenges: Challenge[] = [
  ...
  // Private, user-created — like every non-official challenge, this was set
  // up by a member for their own friend group, not by an admin.
  { id: "chal_002", title: "50km This Month", description: "Walk or run 50 kilometres before the month ends.", goal_steps: 65616, start_date: dateKey(20), end_date: dateKey(-9), badge_emoji: "🗺️", accent_color_value: null, visibility: "private", is_official: false, invite_code: "TOM-50K-88", created_by: "usr_002", created_at: daysAgo(21), image_url: null, rules: "Distance is calculated from steps using each participant's stride length — GPS-tracked sessions are not required.", winner_type: "most_steps", status: "published", winner_user_id: "usr_002", admin_notes: null },
  { id: "chal_003", title: "Weekend Warrior", description: "Get 25,000 steps over Saturday and Sunday.", goal_steps: 25000, start_date: dateKey(4), end_date: dateKey(-2), badge_emoji: "⚡", accent_color_value: null, visibility: "private", is_official: false, invite_code: "DAN-WKND-04", created_by: "usr_006", created_at: daysAgo(5), image_url: null, rules: "Most total steps over the weekend wins.", winner_type: "most_steps", status: "published", winner_user_id: "usr_006", admin_notes: null },
...
export const mockBadges: Badge[] = [
  ...
  // Retired — "early morning session" isn't one of the app's three real
  // achievement categories (Steps / Streaks / Challenges), so this can't be
  // recreated, but past awards stand and it stays visible in the catalog.
  { id: "badge_002", name: "Early Bird", description: "Logged a walk before 7am, five times.", emoji: "🌅", requirement_metric: "early_morning_sessions", requirement_value: 5, enabled: false, visible: true, created_at: daysAgo(280) },
  ...
  // Retired — same reason as Early Bird above: "community posts" isn't one
  // of the three real achievement categories.
  { id: "badge_005", name: "Community Champion", description: "Posted 25 times and helped others stay motivated.", emoji: "💬", requirement_metric: "community_posts", requirement_value: 25, enabled: false, visible: true, created_at: daysAgo(200) },
...
// --- Analytics events (365-day synthetic series for charts) --------------------

// Covers a full year so the Overview DAU chart's 30/90/365-day toggle always
// has real data to show, not just the most recent month.
const ANALYTICS_HISTORY_DAYS = 365;
...
function buildAnalyticsEvents(): AnalyticsEvent[] {
  ...
  for (let day = ANALYTICS_HISTORY_DAYS - 1; day >= 0; day--) {
    // Weekly rhythm: lower on Sun/Mon (day index churns through a 7-cycle), gentle upward trend over the year.
    const weekday = (day + 3) % 7;
    const weekendDip = weekday === 0 || weekday === 6 ? 0.78 : 1;
    const trend = 1 + (ANALYTICS_HISTORY_DAYS - 1 - day) * 0.0025;
...
// --- Support tickets (synthetic — no real helpdesk pipeline yet) -------------
