...
export interface UserProfile {
  ...
  // Set for a timed mute (e.g. "Mute 24 hours" from a report); null means
  // either not muted, or muted with no expiry (a manual "Ban from posting").
...
// There's no real helpdesk pipeline (Zendesk/Intercom) wired in yet — this
// type exists purely so the Dashboard's "Needs attention" section has a real
// shape to point at once one is. Mock data only for now.
...
// A device that reported (or was reported as) a failed OTA firmware push —
// distinct from `Device.firmware_version`, which only tracks the version
// currently installed, not failed attempts to change it.
