...
/// Shares either today's live step progress (the home/stats entry point) or
/// one specific completed workout (the session-summary entry point) — same
/// destination picker, composer, and branded social card either way, just
/// fed from different data depending on which one was passed in.
class ShareStepsScreen extends ConsumerStatefulWidget {
  const ShareStepsScreen({super.key, this.session});

  /// When set, this screen shares this workout instead of today's steps.
...
class _ShareStepsScreenState extends ConsumerState<ShareStepsScreen> {
  ...
  static const _workoutQuickCaptions = [
    'Crushed it today! 💪',
    'Feeling strong 🔥',
  ...
  bool get _isWorkout => widget.session != null;
  ...
  Widget build(BuildContext context) {
    ...
    final accent = _isCommunity
        ? AppColors.accent
        : brandColorOf(_selected.brand);
  ...
  AppBar _buildAppBar(BuildContext context) {
    return AppBar(
      ...
      title: Text(
        _isWorkout ? 'Share Workout' : 'Share Your Steps',
    ...
    return Column(
      ...
      children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Share your workout, celebrate your progress, '
                      'and motivate your community.',
                      style: AppTypography.bodyS,
                    ),
                  ],
        ...
        Wrap(
          spacing: AppTheme.spaceS,
          runSpacing: AppTheme.spaceS,
          children: _workoutQuickCaptions.map((cap) {
            final isActive = _message == cap;
            return GestureDetector(
              onTap: () {
                HapticsHelper.lightImpact();
    ...
    return Stack(
      children: [
        TextField(
          ...
          // Matches the limit on the actual community post composer
          // (community_screen.dart) — this message becomes that same post.
          maxLength: 1000,
          maxLines: 3,
          minLines: 3,
        ...
        Positioned(
          ...
          child: Text('$_charCount/1000', style: AppTypography.labelS),
        ),
      ],
    );
  }
